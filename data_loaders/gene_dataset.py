...
class GenePytorchDataset(Dataset):
    """
    ...
    def __init__(self,
        ...
        self.ds_conf = DatasetConfig('gene', tp)  # used for paths
...
class GeneDataset(SummReviewDataset):
    """
    Main class for using Yelp dataset
    """
    def __init__(self, tp=0):
        super(GeneDataset, self).__init__()
        self.name = 'gene_%s' % tp
        self.conf = DatasetConfig('gene', tp)
...
if __name__ == '__main__':
    ...
    hp = HParams()
    tp = 0
    ds = SummDatasetFactory.get('gene', tp)
